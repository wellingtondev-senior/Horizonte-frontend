...
export function UserAuthForm({ onClick, className, ...props }: UserAuthFormProps) {
    ...
    <div className={cn("grid gap-6", className)} {...props}>
      <form className="px-20 max-lg:px-4 py-10 flex flex-col gap-10" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div>
            <div className="flex items-center justify-between border border-gray-200 rounded-lg h-[40px]">
              <input
                ...
                className="w-full border-transparent focus:border-none focus:border-transparent focus:outline-none focus:ring-0 ring-0 rounded-r-lg px-2 h-[38px] rounded-lg"
              />
              <Button type="button" onClick={() => setIsVisiblePass(!isVisiblePass)} variant="ghost" className="w-[40px] h-[40px] bg-trasnparent rounded-l-lg flex items-center justify-center p-0 hover:bg-transparent">
                {
                  isVisiblePass ?
                    <GoEye className="w-6 h-6 fill-gray-800s" /> :
                    <GoEyeClosed className="w-6 h-6 fill-gray-800s" />
                }
        ...
        <Button type="submit" className="w-full bg-[#F98E1B] rounded-full h-[40px]">
          {
            authenticacao.isPending ? <Spinner /> : <span className="font-bold tracking-[0.2px] text-[16px]">Entrar</span>
          }
        </Button>
        <div className="flex flex-col items-center justify-center gap-4">
          <Button type="button" onClick={() => pageStore.setPage(PageState.LEMBRARSENHA, 2)} variant="ghost" className="text-[14px] text-[#F98E1B] font-bold hover:bg-transparent p-0 h-[15px]">Recuperar a senha</Button>
          <div className="flex gap-2 items-center">
            <span className="text-[14px] text-gray-700 font-bold">Você não tem conta?</span>
            <Button type="button" onClick={() => pageStore.setPage(PageState.REGISTRO, 1)} variant="ghost" className="text-[14px] text-[#F98E1B] font-bold hover:bg-transparent p-0 h-[14px]">Efetuar cadastro</Button>
          </div>
        </div>
      </form>
